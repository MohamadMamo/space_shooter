...
# Fonts (built once and reused, instead of re-created every draw call)
FONT_40 = pygame.font.SysFont("comicsans", 40)
FONT_60 = pygame.font.SysFont("comicsans", 60)
FONT_30 = pygame.font.SysFont("comicsans", 30)

# Contact damage is ignored for this long (ms) after a hit, so overlapping
# with an enemy/boss for multiple frames doesn't drain health instantly.
CONTACT_INVULNERABILITY_MS = 500
...
def main():
    ...
    def redraw_window():
        ...
        score_text = FONT_40.render(f"Score: {score}", True, WHITE)
        level_text = FONT_40.render(f"Level: {level}", True, WHITE)
        high_score_text = FONT_40.render(f"High Score: {high_score}", True, WHITE)
    ...
    def game_over_screen():
        ...
        level_text = FONT_40.render(f"Level: {level}", True, WHITE)
        high_score_text = FONT_40.render(f"High Score: {high_score}", True, WHITE)
        restart_text = FONT_30.render("Press R to Restart", True, WHITE)
        ...
        while run:
            ...
            for enemy in enemies[:]:
                ...
                if enemy.rect.top > HEIGHT:
                    ...
                    if now - spaceship.last_hit_time >= CONTACT_INVULNERABILITY_MS:
                        spaceship.last_hit_time = now
                        spaceship.health -= 10
                ...
                if boss.rect.colliderect(spaceship.rect):
                    ...
                    if spaceship.health <= 0:
                        ...
                        if not run:
                            ...
                            combo = 1
                            spawn_enemies()
                            # Not inside a for-loop over a mutated collection here (unlike
                            # the enemy-restart above), so a bare `break` would exit the
                            # outer `while run:` loop and end the game instead of
                            # restarting it. `continue` gets the equivalent safety -- skip
                            # the rest of this frame's processing of now-stale state --
                            # without doing that.
                            continue
                else:
                    for bullet in bullets[:]:
